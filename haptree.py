...
#This file runs by executing 'python haptree.py data_phased.vcf result.phase'
...
for line in VCF_file: 
	...
	if genotypes[0] == '0|1': 
		snptable[(var[0],int(var[1]),'c')] = [0,1,genotypes,1] 
		phased +=1 
	elif genotypes[0] == '1|0': 
		snptable[(var[0],int(var[1]),'c')] = [1,0,genotypes,1] 
		phased +=1 
	elif genotypes[0] == '0/1' or genotypes[0] == '1/0': 
		snptable[(var[0],int(var[1]),'c')] = [-1,-1,genotypes,0] 
		unphased +=1 
	else: pass 
...
for line in hapcut:
	if '****' in line: 
		FINAL_STATS[0] += phased-1 
		FINAL_STATS[1] += switches
		FINAL_STATS[2] += phased-2 
		FINAL_STATS[3] += bitflips
		...
		try: 
			...
			if snp[0] != -1 and var[1] != '-': ## phased in VCF file and hapcut output
				...
				flag = 0
				if variants ==0 and snp[0] == int(var[1]): 
					m = 0 
					runlength = 1
				elif variants ==0 and snp[0] == int(var[2]): 
					m = 1 
					runlength = 1 
